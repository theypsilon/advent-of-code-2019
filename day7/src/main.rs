...
fn amplifiers_part1(instructions: Instructions, phase: PhaseSetting) -> i64 {
    ...
    for digit in phase {
        ...
        input.extend(output);
        let computation = Computer::with_input(instructions.clone(), input).run_until_halt();
...
fn amplifiers_part2(instructions: Instructions, phase: PhaseSetting) -> i64 {
    let mut computers: Vec<Computer> = (0..5)
    ...
    'feedback: loop {
        for (i, computer) in computers.iter_mut().enumerate() {
            computer.set_input(if first_loop {
                vec![phase[i], output]
            } else {
                vec![output]
            });
...
type ComputerInput = Vec<i64>;
type ComputerOutput = Vec<i64>;
...
impl Computer {
    ...
    pub fn with_input(instructions: Instructions, input: ComputerInput) -> Self {
        Computer {
            instructions,
            input,
            index: 0,
    ...
    pub fn set_input(&mut self, input: ComputerInput) {
        self.input = input;
    }

    pub fn run_until_halt(mut self) -> (Instructions, ComputerOutput) {
        let mut outputs = vec![];
        while let ComputerState::Paused(value) = self.run_until_output() {
            outputs.push(value);
        }
        (self.instructions, outputs)
    }

    pub fn run_until_output(&mut self) -> ComputerState {
        loop {
            let op = self.opcode();
            match op.de {
                1 => {
                ...
                99 => return ComputerState::Halted,
...
mod test {
    ...
    fn computer_one(instructions: Instructions) -> (Instructions, ComputerOutput) {
        computer_n(instructions, 1)
    }

    fn computer_n(instructions: Instructions, input: i64) -> (Instructions, ComputerOutput) {
        Computer::with_input(instructions, vec![input]).run_until_halt()
    }
    fn get_value(instructions: Instructions, mode: i64, value: i64) -> i64 {
        Computer::with_input(instructions, vec![0]).get(mode, value)
    }
    ...
    eq_tests! {
        computer_one_1: stringify(computer_one(vec![1, 0, 0, 0, 99]).0) => "2,0,0,0,99";
        computer_one_2: stringify(computer_one(vec![2, 3, 0, 3, 99]).0) => "2,3,0,6,99";
        computer_one_3: stringify(computer_one(vec![2, 4, 4, 5, 99, 0]).0) => "2,4,4,5,99,9801";
        computer_one_4: stringify(computer_one(vec![1, 1, 1, 4, 99, 5, 6, 0, 99]).0) => "30,1,1,4,2,5,6,0,99";
        ...
        computer_n_01: computer_n(vec![3,9,8,9,10,9,4,9,99,-1,8], 8).1[0] => 1;
        computer_n_02: computer_n(vec![3,9,8,9,10,9,4,9,99,-1,8], 7).1[0] => 0;

        thruster_example_1: amplifiers_part1(vec![3,15,3,16,1002,16,10,16,1,16,15,15,4,15,99,0,0], vec![4,3,2,1,0]) => 43210;
        thruster_example_2: amplifiers_part1(vec![3,23,3,24,1002,24,10,24,1002,23,-1,23,
            101,5,23,23,1,24,23,23,4,23,99,0,0], vec![0, 1, 2, 3, 4]) => 54321;
        thruster_example_3: amplifiers_part1(vec![3,31,3,32,1002,32,10,32,1001,31,-2,31,1007,31,0,33,
        ...
        test_example_4: amplifiers_part2(vec![3,26,1001,26,-4,26,3,27,1002,27,2,27,1,27,26,
            27,4,27,1001,28,-1,28,1005,28,6,99,0,0,5], vec![9,8,7,6,5]) => 139629729;
        test_example_5: amplifiers_part2(vec![3,52,1001,52,-5,52,3,53,1,52,56,54,1007,54,5,55,1005,55,26,1001,54,
